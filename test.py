...
def train(args, epoch, train_queue, valid_queue, net, alphas, criterion, optimizer, lr, record_file=None):
    ...
    for batch_index, ((images, labels, index), (images_valid, labels_valid, index_valid)) in enumerate(
        ...
        if epoch >= args.alpha_epoch:
            alphas.step(images, labels, images_valid, labels_valid, lr, optimizer, indices, eta_min=args.lr_min)
        ...
        if batch_index % args.print_freq == 0:
            # curr_time = time.time()
            print('epoch {}, train {}/{}, loss: {:.3f}, top1: {:.3f}, top5: {:.3f}, lr: {:.3f}'.format(epoch, batch_index,
                                                                                           len(train_queue),
                                                                                           objs.avg, top1.avg,
                                                                                           top5.avg,lr))
